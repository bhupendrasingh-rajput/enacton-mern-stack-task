...
function EditProduct({ params }: { params: { id: string } }) {
  ...
  const {
    ...
    onSubmit: async (values, actions) => {
      try {
        const occArr = values.occasion.map(item => item.value).join(',');
        const brandArr = values.brands.map(item => item.value);
        const catArr = values.categories.map(item => item.value);
        const price = (values.old_price - (values.old_price * (values.discount / 100))).toFixed(2);
        const productData = {
          name: values.name,
          ...
          brands: JSON.stringify(brandArr),
        ...
        let result = await updateProduct(id, productData);
        await updateProductCategories(id, catArr);
